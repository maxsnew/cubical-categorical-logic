...
module _ {C : Category ℓ ℓ'} where
  ...
  idCoHom D .snd .V-δ = makeNatTransPath (funExt λ x → ⋆IdL _ ∙ sym (⋆IdR _) ∙ cong₂ _⋆_ refl (sym (D .fst .F-id) ∙ sym (⋆IdR _)))

  compCoHom : {D D' D'' : Comonad C} → ComonadHom D' D'' → ComonadHom D D' → ComonadHom D D''
  compCoHom ϕ ψ .fst = ϕ .fst ∘ᵛ ψ .fst
  compCoHom ϕ ψ .snd .V-ε = makeNatTransPath (funExt λ x → ⋆Assoc _ _ _ ∙ cong₂ _⋆_ refl (λ i → ϕ .snd .V-ε i ⟦ _ ⟧) ∙ (λ i → ψ .snd .V-ε i ⟦ _ ⟧))
  compCoHom {D}{D'}{D''} ϕ ψ .snd .V-δ = makeNatTransPath (funExt λ x →
    ⋆Assoc _ _ _ ∙ cong₂ _⋆_ refl (λ i → ϕ .snd .V-δ i ⟦ _ ⟧) ∙ sym (⋆Assoc _ _ _) ∙ cong₂ _⋆_ ((λ i → ψ .snd .V-δ i ⟦ _ ⟧)) refl
    ∙ (⋆Assoc _ _ _ ∙ cong₂ _⋆_ refl (⋆Assoc _ _ _ ∙ cong₂ _⋆_ refl (sym (⋆Assoc _ _ _) ∙ cong₂ _⋆_ (sym (ψ .fst .N-hom _)) refl)) )
    ∙ lem) where
    open import Cubical.Tactics.FunctorSolver.Reflection
    lem : ∀ {x} → snd D .IsComonad.δ .N-ob x ⋆ F-hom (fst D) (N-ob (ψ .fst) x) ⋆ (fst D .F-hom (N-ob (ϕ .fst) x) ⋆ ψ .fst .N-ob (F-ob (D'' .fst) x)) ⋆ N-ob (ϕ .fst) (F-ob (D'' .fst) x) ≡ snd D .IsComonad.δ .N-ob x ⋆ F-hom (fst D) (ψ .fst .N-ob x ⋆ ϕ .fst .N-ob x) ⋆ ψ .fst .N-ob (F-ob (D'' .fst) x) ⋆ ϕ .fst .N-ob (F-ob (D'' .fst) x)
