...
module _ {C : Category ℓ ℓ'} {T T' : Monad C} (ϕ : MonadHom T T') where
  ...
  push .F-seq {x}{y}{z} f g = lem' ∙ cong₂ (seq' C) refl (((cong₂ (seq' C) refl (funExt⁻ (cong N-ob (ϕ .snd .N-μ)) _) ∙ sym lem) ∙ cong₂ (comp' C) refl (ϕ .fst .N-hom _)) ∙ C .⋆Assoc _ _ _) ∙ (sym (C .⋆Assoc _ _ _)) where
    lem : (C ⋆ (C ⋆ fst T .F-hom ((C ⋆ g) (N-ob (ϕ .fst) z))) (ϕ .fst .N-ob (F-ob (T' .fst) z))) (N-ob (IsMonad.μ (T' .snd)) z)
          ≡ T .fst ⟪ g ⟫ ⋆⟨ C ⟩ (T .fst ⟪ ϕ .fst ⟦ _ ⟧ ⟫ ⋆⟨ C ⟩ ϕ .fst ⟦ _ ⟧ ⋆⟨ C ⟩ (T' .snd .μ ⟦ _ ⟧))
    lem = solveFunctor! C C (T .fst)

    lem' : (C ⋆ (C ⋆ f) ((C ⋆ F-hom (T .fst) g) (N-ob (μ (T .snd)) z))) (N-ob (ϕ .fst) z) ≡ (C ⋆ f) ((C ⋆ F-hom (T .fst) g) ((C ⋆ snd T .μ .N-ob z) (ϕ .fst .N-ob z)))
    lem' = solveCat! C
