-- Free functor between categories generated from
-- two graphs and a function on nodes between them
...
open import Cubical.Foundations.Id hiding (_≡_; isSet; subst)
  renaming (refl to reflId; _∙_ to _∙Id_;
    transport to transportId; funExt to funExtId)
...
funExtRefl : ∀ {A : Type ℓ}{B : A → Type ℓ'} {f} →
             Path (Id f f) (funExtId {A = A}{B = B}{f = f}
               (λ x → reflId)) reflId
...
substIdToPath {A = A}{B}{x = x} p =
  funExt λ bx → J (λ by q → subst B (idToPath q) bx ≡
    transportId B q bx) (lem bx) p
  where
...
module FreeFunctor (G : Graph ℓg ℓg')
  ...
  module _ {𝓒 : Category ℓc ℓc'}{𝓓 : Category ℓd ℓd'} {𝓕 : Functor 𝓒 𝓓} where
    module Semantics (ıG : Interp G 𝓒) (ıH : Interp H 𝓓)
      ...
      semH-hom (F⟪_⟫ {A}{B} x) =
        transportId (λ (f : G .Node → 𝓓 .ob) →
          𝓓 [ f A , f B ]) ıϕ (𝓕 ⟪ semG ⟪ x ⟫ ⟫)
      -- preserves 1-cells
      ...
      semH-hom (F-idₑ {A} i) = unbound i
        where
          unbound : transportId
                    (λ f → 𝓓 [ f A , f A ]) ıϕ (𝓕 ⟪ semG ⟪ idₑ ⟫ ⟫) ≡ 𝓓 .id
          unbound = J (λ g p →
            transportId (λ f → 𝓓 [ f A , f A ]) p
              (𝓕 ⟪ semG ⟪ idₑ ⟫ ⟫) ≡ 𝓓 .id) ((𝓕 ∘F semG) .F-id) ıϕ
      semH-hom (F-seqₑ {A}{B}{C} e e' i) = unbound i
        where
          unbound : transportId
                    (λ f → 𝓓 [ f A , f C ]) ıϕ (𝓕 ⟪ semG ⟪ e ⋆ₑ e' ⟫ ⟫)
                    ≡ (transportId
                      (λ f → 𝓓 [ f A , f B ])
                      ıϕ (𝓕 ⟪ semG ⟪ e ⟫ ⟫)) ⋆⟨ 𝓓 ⟩
                        (transportId
                        (λ f → 𝓓 [ f B , f C ]) ıϕ (𝓕 ⟪ semG ⟪ e' ⟫ ⟫))
          unbound = J (λ g p →
            transportId
            (λ f → 𝓓 [ f A , f C ]) p (𝓕 ⟪ semG ⟪ e ⋆ₑ e' ⟫ ⟫) ≡
      ...
      semH-hom (isSetFExp f g p q i j) =
        𝓓 .isSetHom (semH-hom f) (semH-hom g)
          (cong semH-hom p) (cong semH-hom q) i j
      ...
      semϕ =
        pathToId (FreeCatG.induction (funcComp 𝓕 semG)
          (funcComp semH Fϕ) (GrHom≡ aoo aoe)) where
        𝓕G = (𝓕 .F-ob ∘f ıG ._$g_)
        Hϕ = (ıH ._$g_ ∘f ϕ)
        ...
        aoe {v}{w} e = toPathP lem where
          lem : Path _
                (transportPath
                (λ i → 𝓓 [ aoo-gen v 𝓕G Hϕ ıϕ i ,
                           aoo-gen w 𝓕G Hϕ ıϕ i ]) (𝓕 ⟪ ıG <$g> e ⟫))
                (transportId   (λ f → 𝓓 [ f v , f w ]) ıϕ (𝓕 ⟪ ıG <$g> e ⟫))
          lem = J (λ f p → Path _
                  ((transportPath (λ i →
                    𝓓 [ aoo-gen v 𝓕G f p i ,
                        aoo-gen w 𝓕G f p i ]) (𝓕 ⟪ ıG <$g> e ⟫)))
                  ((transportId   (λ f →
                    𝓓 [ f v , f w ]) p (𝓕 ⟪ ıG <$g> e ⟫))))
                  (transportRefl (𝓕 ⟪ ıG <$g> e ⟫))
                  ıϕ

      module Uniqueness (arb𝓒 : Functor FG 𝓒)
                        (arb𝓓 : Functor FH 𝓓)
                        (arb𝓕 : Path (Functor FG 𝓓)
                                      (𝓕 ∘F arb𝓒)
                                      (arb𝓓 ∘F Fϕ))
                -- arb𝓕 .F-ob : Id {G .Node → 𝓓 .ob} ((𝓕 ∘F arb𝓒) ∘Interp ηG)
        ...
        sem-uniq-H = Functor≡ aoo aom where
          ...
          aom-type {v}{w} f =
            PathP (λ i → 𝓓 [ aoo v i , aoo w i ]) (arb𝓓 ⟪ f ⟫) (semH ⟪ f ⟫)
          ...
          aom-F : ∀ {v w}
                ...
                → PathP (λ i → 𝓓 [ (arb𝓓-agree i $g (ϕ v)) ,
                        ...
                        (arb𝓓 ⟪ Fϕ ⟪ e ⟫ ⟫)
                        (transportId (λ (f : G .Node → 𝓓 .ob) →
                          𝓓 [ f v , f w ]) ıϕ (𝓕 ⟪ semG ⟪ e ⟫ ⟫))
          aom-F {v}{w} e =
            pathified ▷ λ i → substIdToPath {B = λ (f : G .Node → 𝓓 .ob) →
                                      𝓓 [ f v , f w ]} ıϕ i (𝓕 ⟪ semG ⟪ e ⟫ ⟫)
            where
              pathified : PathP
                (λ i → 𝓓 [ arb𝓓-agree i $g ϕ v , arb𝓓-agree i $g ϕ w ])
                (arb𝓓 ⟪ Fϕ ⟪ e ⟫ ⟫)
                (transportPath (λ i → 𝓓 [ ıϕp i v ,
                                           ıϕp i w ]) (𝓕 ⟪ semG ⟪ e ⟫ ⟫))
              pathified = toPathP⁻ ((
                fromPathP⁻ lem'
                ∙ cong (transport⁻ (λ i →
                  𝓓 [ arb𝓕 (~ i) ⟅ v ⟆ ,
                      arb𝓕 (~ i) ⟅ w ⟆ ])) (fromPathP⁻ lem𝓒)
                ∙ sym (transportComposite ((λ i →
                  𝓓 [ 𝓕 ⟅ arb𝓒-agree (~ i) $g v ⟆ ,
                      𝓕 ⟅ arb𝓒-agree (~ i) $g w ⟆  ]))
                  (λ i → 𝓓 [ arb𝓕 i ⟅ v ⟆ , arb𝓕 i ⟅ w ⟆  ])
                    ((𝓕 ⟪ semG ⟪ e ⟫ ⟫)))
                ∙ ((λ i → transport (substOf-sems-agreeϕ i) (𝓕 ⟪ semG ⟪ e ⟫ ⟫)))
                ∙ transportComposite (λ i →
                  𝓓 [ ıϕp i v , ıϕp i w ])
                  (λ i → 𝓓 [ arb𝓓-agree (~ i) $g ϕ v ,
                             arb𝓓-agree (~ i) $g ϕ w ]) (𝓕 ⟪ semG ⟪ e ⟫ ⟫)
                ))
                where
                  ...
                  substOf-sems-agreeϕ =
                    sym (cong-∙ A
                      (λ i x → 𝓕 ⟅ arb𝓒-agree (~ i) $g x ⟆)
                        λ i x → arb𝓕 i ⟅ x ⟆)
                    -- Square (λ i z → ıϕ i z) (λ i → _⟅_⟆ (semϕ' i))
                    -- (λ i z → 𝓕 ⟅ sems-agree𝓒 (~ i) $g z ⟆)
                    -- (λ i x → sems-agree𝓓 (~ i) $g ϕ x)
                    ∙ cong (cong A)
          ...
          aom (⋆ₑIdL f i) =
            isSet→SquareP (λ i j → 𝓓 .isSetHom) (aom-seq aom-id (aom f))
              (aom f) (λ i → arb𝓓 ⟪ ⋆ₑIdL f i ⟫) (λ i → (semH ⟪ ⋆ₑIdL f i ⟫)) i
          aom (⋆ₑIdR f i) =
            isSet→SquareP (λ i j → 𝓓 .isSetHom) (aom-seq (aom f) aom-id)
              (aom f ) (λ i → arb𝓓 ⟪ ⋆ₑIdR f i ⟫) (λ i → semH ⟪ ⋆ₑIdR f i ⟫) i
          aom (⋆ₑAssoc f f₁ f₂ i) =
            isSet→SquareP (λ i j → 𝓓 .isSetHom)
              (aom-seq (aom-seq (aom f) (aom f₁)) (aom f₂))
              (aom-seq (aom f) (aom-seq (aom f₁) (aom f₂)))
              (λ i → arb𝓓 ⟪ ⋆ₑAssoc f f₁ f₂ i ⟫)
              (λ i → semH ⟪ ⋆ₑAssoc f f₁ f₂ i ⟫) i
          aom (F-idₑ i) =
            isSet→SquareP (λ i j → 𝓓 .isSetHom) (aom-F idₑ) aom-id
            (λ i → arb𝓓 ⟪ F-idₑ i ⟫) (λ i → semH ⟪ F-idₑ i ⟫) i
          aom (F-seqₑ f g i) =
            isSet→SquareP (λ i j → 𝓓 .isSetHom) (aom-F (f ⋆ₑ g))
            (aom-seq (aom-F f) (aom-F g))
            (λ i → arb𝓓 ⟪ F-seqₑ f g i ⟫) (λ i → semH ⟪ F-seqₑ f g i ⟫) i
          aom (isSetFExp f f₁ x y i j) k =
            isSet→SquareP
              (λ i j →
                (isOfHLevelPathP {A = λ k → 𝓓 [ aoo _ k , aoo _ k ]} 2
                  (𝓓 .isSetHom) (arb𝓓 ⟪ isSetFExp f f₁ x y i j ⟫)
