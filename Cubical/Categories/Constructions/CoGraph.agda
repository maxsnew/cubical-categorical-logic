...
module _ {C : Category ℓC ℓC'}{D : Category ℓD ℓD'} (P : C o-[ ℓS ]-* D) where
  ...
  private
    ...
    ↑o-r-retract : ∀ {A B} → (f : CoGraphHom (inl A) (inl B)) → ↑o (↑o-r f) ≡ f
    ↑o-r-retract (↑o f) = refl

    ↑*-r : ∀ {A B} → CoGraphHom (inr A) (inr B) → D [ A , B ]
    ↑*-r (↑* g) = g

    ↑*-r-retract : ∀ {A B} → (f : CoGraphHom (inr A) (inr B)) → ↑* (↑*-r f) ≡ f
    ↑*-r-retract (↑* f) = refl

    ↑p-r-retract : ∀ {A B} → (f : CoGraphHom (inl A) (inr B)) → ↑p (↑p-r f) ≡ f
