...
import Cubical.Categories.Displayed.Fibration.Base as Categoryᴰ
...
module _ {C : Category ℓC ℓC'}{x₁ x₂ : C .ob}
  ...
  module _ {xᴰ₁ : Cᴰ.ob[ x₁ ]}{xᴰ₂ : Cᴰ.ob[ x₂ ]}
    (lift-π₁ : Categoryᴰ.CartesianLift Cᴰ xᴰ₁ c×c'.π₁)
    (lift-π₂ : Categoryᴰ.CartesianLift Cᴰ xᴰ₂ c×c'.π₂)
    (vbp : BinProductⱽ Cᴰ ((lift-π₁ .Categoryᴰ.CartesianLift.f*yᴰ) , (lift-π₂ .Categoryᴰ.CartesianLift.f*yᴰ)))
    where
    ...
    BinProductⱽ→BinProductᴰ =
      BinProductⱽ→PshProdReprᴰ prod
        ...
        vbp
module _ {C : Category ℓC ℓC'} (Cᴰ : Categoryᴰ C ℓCᴰ ℓCᴰ')
  (cartesianLifts : Categoryᴰ.isFibration Cᴰ)
