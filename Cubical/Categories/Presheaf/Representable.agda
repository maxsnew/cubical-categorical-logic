...
module _ {ℓo}{ℓh}{ℓp} (C : Category ℓo ℓh) (P : Presheaf C ℓp) where
  ...
  record isUniversal
    (η : Elementᴾ {C = C} P) : Type (ℓ-max (ℓ-max ℓo ℓh) ℓp) where
    private
      vertex = η .fst
      element = η .snd
    field
      ...
      is-uniq : ∀ {b} (ϕ : (P ⟅ b ⟆) .fst) f →
                (C [ element ∘ᴾ⟨ P ⟩ f ] ≡ ϕ) → f ≡ coinduction ϕ
    ...
    η-expansion : ∀ {b} (f : C [ b , vertex ]) →
                  f ≡ coinduction (C [ element ∘ᴾ⟨ P ⟩ f ])
    ...
    is-uniq-converse : ∀ {b} (ϕ : (P ⟅ b ⟆) .fst) f →
                       f ≡ coinduction ϕ → C [ element ∘ᴾ⟨ P ⟩ f ] ≡ ϕ
    ...
    coinduction-natural : ∀ {a b} → (ϕ : (P ⟅ b ⟆) .fst) → (f : C [ a , b ])
                        → coinduction ϕ ∘⟨ C ⟩ f ≡
                            coinduction (C [ ϕ ∘ᴾ⟨ P ⟩ f ])
    coinduction-natural ϕ f =
      is-uniq (C [ ϕ ∘ᴾ⟨ P ⟩ f ]) (coinduction ϕ ∘⟨ C ⟩ f)
  ...
  private
    ...
    univEltLemma : ∀ {η η' : Elementᴾ {C = C} P} → (η-univ : isUniversal η) →
                  (η'-univ : isUniversal η') →
    ...
    univEltLemma {η}{η'} η-univ η'-univ =
      ...
      lem : (C [ η' .snd ∘ᴾ⟨ P ⟩ η'-univ .coinduction (η .snd) ∘⟨ C ⟩
               η-univ .coinduction (η' .snd) ]) ≡ η' .snd
      lem =
        (C [ η' .snd ∘ᴾ⟨ P ⟩ η'-univ .coinduction (η .snd) ∘⟨ C ⟩
           η-univ .coinduction (η' .snd) ])
          ≡⟨ ∘ᴾAssoc C P _ _ _ ⟩
        (C [ C [ η' .snd ∘ᴾ⟨ P ⟩ η'-univ .coinduction (η .snd) ] ∘ᴾ⟨ P ⟩
           η-univ .coinduction (η' .snd) ])
          ≡[ i ]⟨ C [ η'-univ .commutes (η .snd) i ∘ᴾ⟨ P ⟩
             η-univ .coinduction (η' .snd) ] ⟩
  ...
  univEltIso {η}{η'} η-univ η'-univ .snd .isIso.sec =
    univEltLemma η-univ η'-univ
  univEltIso {η}{η'} η-univ η'-univ .snd .isIso.ret =
    univEltLemma η'-univ η-univ
  ...
  isUniversal→isTerminalElement : ∀ {η : Elementᴾ {C = C} P} →
                                  isUniversal η → isTerminal Elements η
  ...
  isTerminalElement→YoIso ((A , η) , η-univ) =
    ...
    pointwise : ∀ c → Cubical.Categories.Category.isIso
                      (SET (ℓ-max ℓh ℓp))
    ...
    pointwise c .isIso.ret =
      funExt (λ f i →
        lift (
          η-univ (_ , C [ η ∘ᴾ⟨ P ⟩ f .lower ]) .snd (f .lower , refl) i .fst))

  YoIso→isTerminalElement : ∀ A
                          → (i : PshIso C (C [-, A ]) P)
                          → isTerminal Elements
                            (A , (i .fst .N-ob A (lift (C .id)) .lower))
  ...
  YoIso→isTerminalElement A (YoA→P , isiso P→YoA sec ret) (B , ϕ) .fst .snd =
    ...
    (YoA→P .N-ob B (lift (C .id ∘⟨ C ⟩ P→YoA .N-ob B (lift ϕ) .lower))) .lower
      ≡[ i ]⟨ YoA→P .N-hom (P→YoA .N-ob B
        (lift ϕ) .lower) i (lift (C .id)) .lower ⟩
    C [ YoA→P .N-ob A (lift (C .id)) .lower ∘ᴾ⟨ P ⟩
      P→YoA .N-ob B (lift ϕ) .lower ] ∎
  YoIso→isTerminalElement A (YoA→P , isiso P→YoA sec ret) (B , ϕ)
    .snd f+ @ (f , ϕ=η∘f) =
      ∫ᴾhomEqSimpl {C = C} P ((P→YoA .N-ob B (lift ϕ) .lower) , _) f+
    (P→YoA .N-ob B (lift ϕ) .lower
      ≡[ i ]⟨ P→YoA .N-ob B (lift (ϕ=η∘f i)) .lower ⟩
    P→YoA .N-ob B
      (lift (C [ YoA→P .N-ob A (lift (C .id)) .lower ∘ᴾ⟨ P ⟩ f ])) .lower
  ...
  sameUMP→CatIso : (u : UnivElt) → (v : UnivElt) →
                   CatIso C (u .vertex) (v .vertex)
  sameUMP→CatIso u v =
...
selfRepresents : (C : Category ℓ ℓ') → (A : C .ob) →
                 Representation C (C [-, A ])
