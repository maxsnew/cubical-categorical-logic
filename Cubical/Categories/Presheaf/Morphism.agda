...
module _ {C : Category ℓc ℓc'}{D : Category ℓd ℓd'}
         (F : Functor C D) (P : Presheaf C ℓp) (Q : Presheaf D ℓq) where
  PshHom : Type (ℓ-max (ℓ-max (ℓ-max ℓc ℓc') ℓp) ℓq)
  PshHom = PresheafCategory C (ℓ-max ℓp ℓq) [ LiftF {ℓp}{ℓq} ∘F P ,
                                              LiftF {ℓq}{ℓp} ∘F Q ∘F (F ^opF) ]
  ...
  module _ (h : PshHom) where
    ...
    preserves-representability : Type (ℓ-max (ℓ-max (ℓ-max (ℓ-max
                                      (ℓ-max ℓc ℓc') ℓd) ℓd') ℓp) ℓq)
