...
module Eval (𝓒 : Category ℓc ℓc') (𝓓 : Category ℓd ℓd')  (𝓕 : Functor 𝓒 𝓓) where
  ...
  solve {A}{B} e e' p = congPath (TautoSem.semH .F-hom)
                                 (isFaithfulPseudoYoneda _ _ _ _ lem) where
    lem : Path _ (PsYo ⟪ e ⟫) (PsYo ⟪ e' ⟫)
    lem = transportPath (λ i → Path _ (Yo-YoSem-Agree (~ i) ⟪ e ⟫)
                        (Yo-YoSem-Agree (~ i) ⟪ e' ⟫)) p
