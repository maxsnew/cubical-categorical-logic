...
-- TODO: we changed the definiton of ×Quiver cod
...
--  open Terminal'Notation FREECC.CCTerminal'
--
--  [ans] : FREECC .fst .ob
--  [ans] = ↑ ans
--
--  [t] [f] : FREECC .fst [ FREECC.𝟙 , [ans] ]
--  [t] = {!!} --(↑ₑ (t,f)) ⋆ₑ π₁
--  [f] = {!!} --(↑ₑ (t,f)) ⋆ₑ π₂
--
--  [t]≠[f] : ¬ ([t] ≡ [f])
...
--      (λ _ _ → isFib→F⟪π₁⟫* (FREECC.CCBinProducts' (_ , _)) _ isFibrationSet ,
--        isFib→F⟪π₂⟫* (FREECC.CCBinProducts' (_ , _)) _ isFibrationSet)
--      (λ _ _ → VerticalBinProds→ϕ[π₁x]∧ψ[π₂x] {F = pts} (FREECC.CCBinProducts' (_ , _))
--        (isFib→F⟪π₁⟫* (FREECC.CCBinProducts' (_ , _)) _ isFibrationSet)
--        (isFib→F⟪π₂⟫* (FREECC.CCBinProducts' (_ , _)) _ isFibrationSet)
